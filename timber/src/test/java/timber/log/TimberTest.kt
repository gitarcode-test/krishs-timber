...
class TimberTest {
    ...
    fun recursion() {
        ...
        assertThrows<IllegalArgumentException> { Timber.plant(timber) }
            .hasMessageThat()
            .isEqualTo("Cannot plant Timber into itself.")
    ...
    fun uprootThrowsIfMissing() {
        assertThrows<IllegalArgumentException> { Timber.uproot(Timber.DebugTree()) }
            .hasMessageThat()
    ...
    // NOTE: this will fail on some future version of Kotlin when lambdas are compiled using
    // invokedynamic
    ...
    private inline fun <reified T : Throwable> assertThrows(body: () -> Unit): ThrowableSubject {
        ...
        throw AssertionError(
            "Expected body to throw ${T::class.java.name} but completed successfully"
        )
    }
